...
from items import BackstagePassItem


def assert_quality_increase(sell_in: int, quality: int, item: BackstagePassItem):
    if sell_in > 10:
...
def test_backstage_pass_quality_increases_by_2(item: BackstagePassItem):
...
@given(item=backstage_pass_factory())
def test_backstage_pass_quality_drops_to_0(item: BackstagePassItem):
    item.sell_in = 0
    gilded_rose = GildedRose([item])
    gilded_rose.update_quality()
    assert item.quality == 0
